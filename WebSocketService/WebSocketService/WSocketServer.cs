...
namespace WebSocketService
{
    public class WSocketServer:IDisposable
    {
        ...
        /// <returns></returns>
        public bool Open(int port, string serverName, bool isUseCertificate = true, string serverStoreName = "", string serverSecurity = "", string serverThumbprint = "")
        {
            ...
            try
            {
                ...
                {
                    Name = serverName,
                    MaxConnectionNumber = 1000, //最大允许的客户端连接数目，默认为100。
                    Mode = SocketMode.Tcp,
                    Port = port, //服务器监听的端口。
                ...
                if (isUseCertificate)
                {
                    ...
                    serverConfig.Certificate = new SuperSocket.SocketBase.Config.CertificateConfig
                    {
                        ...
                        //FilePath = "D:\\sslkey\\IViewWeb.pfx",
                        //Password = "hengsion"
                        FilePath = "xxxxxx.pfx",
                        Password = "xxxxxxxx"
                    };
